...
extern "C" void calc_rhs_SelectBCs(CCTK_ARGUMENTS)
{
  ...
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "GFOffset::evolved_grouprhs","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for GFOffset::evolved_grouprhs.");
  return;
}
...
static void calc_rhs_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL pdphi11 CCTK_ATTRIBUTE_UNUSED = (-2*phiL + 
      GFOffset(phi,-1,0,0) + GFOffset(phi,1,0,0))*pow(dx,-2);
    
    CCTK_REAL pdphi22 CCTK_ATTRIBUTE_UNUSED = (-2*phiL + 
      GFOffset(phi,0,-1,0) + GFOffset(phi,0,1,0))*pow(dy,-2);
    
    CCTK_REAL pdphi33 CCTK_ATTRIBUTE_UNUSED = (-2*phiL + 
      GFOffset(phi,0,0,-1) + GFOffset(phi,0,0,1))*pow(dz,-2);
