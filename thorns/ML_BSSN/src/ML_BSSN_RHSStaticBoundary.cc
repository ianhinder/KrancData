...
static void ML_BSSN_RHSStaticBoundary_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  CCTK_ENDLOOP3STR(ML_BSSN_RHSStaticBoundary);
}
extern "C" void ML_BSSN_RHSStaticBoundary(CCTK_ARGUMENTS)
{
  ...
  AssertGroupStorage(cctkGH, "ML_BSSN_RHSStaticBoundary", 9, groups);
  ...
  LoopOverBoundary(cctkGH, ML_BSSN_RHSStaticBoundary_Body);
  if (verbose > 1)
  {
