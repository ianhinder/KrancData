...
static void ML_BSSN_constraints1_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
    ...
    vecimin,vecimax, CCTK_REAL_VEC_SIZE)
  {
    ...
    CCTK_REAL_VEC trR CCTK_ATTRIBUTE_UNUSED = 
      kmadd(gu11,R11,kmadd(ToReal(2),kmul(gu12,R12),kmadd(ToReal(2),kmul(gu13,R13),kmadd(gu22,R22,kmadd(ToReal(2),kmul(gu23,R23),kmul(gu33,R33))))));
...
extern "C" void ML_BSSN_constraints1(CCTK_ARGUMENTS)
{
  ...
  AssertGroupStorage(cctkGH, "ML_BSSN_constraints1", 8, groups);
  
  switch (fdOrder)
  {
    case 2:
    {
      EnsureStencilFits(cctkGH, "ML_BSSN_constraints1", 1, 1, 1);
      break;
    }
    
    case 4:
    {
      EnsureStencilFits(cctkGH, "ML_BSSN_constraints1", 2, 2, 2);
      break;
    }
    default:
      CCTK_BUILTIN_UNREACHABLE();
  ...
  LoopOverInterior(cctkGH, ML_BSSN_constraints1_Body);
  if (verbose > 1)
  {
