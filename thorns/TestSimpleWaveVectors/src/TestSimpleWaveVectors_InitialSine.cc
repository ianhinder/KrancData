...
static void TestSimpleWaveVectors_InitialSine_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3STR(TestSimpleWaveVectors_InitialSine,
  ...
  CCTK_ENDLOOP3STR(TestSimpleWaveVectors_InitialSine);
}
extern "C" void TestSimpleWaveVectors_InitialSine(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering TestSimpleWaveVectors_InitialSine_Body");
  }
  if (cctk_iteration % TestSimpleWaveVectors_InitialSine_calc_every != TestSimpleWaveVectors_InitialSine_calc_offset)
  {
    return;
  }
