...
extern "C" void calc_rhs_SelectBCs(CCTK_ARGUMENTS)
{
  ...
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "TilingTest::evolved_grouprhs","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for TilingTest::evolved_grouprhs.");
  return;
}
