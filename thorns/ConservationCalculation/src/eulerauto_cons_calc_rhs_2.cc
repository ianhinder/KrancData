...
static void eulerauto_cons_calc_rhs_2_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    const CCTK_REAL PDplus2EnFlux CCTK_ATTRIBUTE_UNUSED = PDplus2(&EnFlux[index]);
...
extern "C" void eulerauto_cons_calc_rhs_2(CCTK_ARGUMENTS)
{
  ...
  AssertGroupStorage(cctkGH, "eulerauto_cons_calc_rhs_2", 8, groups);
  
  EnsureStencilFits(cctkGH, "eulerauto_cons_calc_rhs_2", 1, 1, 1);
  
  LoopOverInterior(cctkGH, eulerauto_cons_calc_rhs_2_Body);
  if (verbose > 1)
  {
