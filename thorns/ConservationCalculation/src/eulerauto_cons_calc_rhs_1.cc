...
extern "C" void eulerauto_cons_calc_rhs_1_SelectBCs(CCTK_ARGUMENTS)
{
  ...
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "ConservationCalculation::Den_grouprhs","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for ConservationCalculation::Den_grouprhs.");
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "ConservationCalculation::En_grouprhs","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for ConservationCalculation::En_grouprhs.");
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "ConservationCalculation::S_grouprhs","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for ConservationCalculation::S_grouprhs.");
  return;
}

static void eulerauto_cons_calc_rhs_1_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    const CCTK_REAL PDplus1DenFlux CCTK_ATTRIBUTE_UNUSED = PDplus1(&DenFlux[index]);
    const CCTK_REAL PDplus1EnFlux CCTK_ATTRIBUTE_UNUSED = PDplus1(&EnFlux[index]);
    const CCTK_REAL PDplus1S1Flux CCTK_ATTRIBUTE_UNUSED = PDplus1(&S1Flux[index]);
    const CCTK_REAL PDplus1S2Flux CCTK_ATTRIBUTE_UNUSED = PDplus1(&S2Flux[index]);
    const CCTK_REAL PDplus1S3Flux CCTK_ATTRIBUTE_UNUSED = PDplus1(&S3Flux[index]);
...
extern "C" void eulerauto_cons_calc_rhs_1(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    ...
    "ConservationCalculation::Den_grouprhs",
    "ConservationCalculation::En_flux_group",
    "ConservationCalculation::En_grouprhs",
    "ConservationCalculation::S1_flux_group",
    "ConservationCalculation::S2_flux_group",
    "ConservationCalculation::S3_flux_group",
    "ConservationCalculation::S_grouprhs"};
  AssertGroupStorage(cctkGH, "eulerauto_cons_calc_rhs_1", 8, groups);
  
  EnsureStencilFits(cctkGH, "eulerauto_cons_calc_rhs_1", 1, 1, 1);
  
  LoopOverInterior(cctkGH, eulerauto_cons_calc_rhs_1_Body);
  if (verbose > 1)
  {
