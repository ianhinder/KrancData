...
extern "C" void TestSimpleWave_CalcRHS_SelectBCs(CCTK_ARGUMENTS)
{
  ...
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "TestSimpleWave::evolved_grouprhs","flat");
  if (ierr < 0)
...
extern "C" void TestSimpleWave_CalcRHS(CCTK_ARGUMENTS)
{
  ...
  AssertGroupStorage(cctkGH, "TestSimpleWave_CalcRHS", 2, groups);
  
  EnsureStencilFits(cctkGH, "TestSimpleWave_CalcRHS", 1, 1, 1);
  
  LoopOverInterior(cctkGH, TestSimpleWave_CalcRHS_Body);
  if (verbose > 1)
  {
