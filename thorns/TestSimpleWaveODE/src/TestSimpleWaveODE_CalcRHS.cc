...
namespace TestSimpleWaveODE {

extern "C" void TestSimpleWaveODE_CalcRHS_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % TestSimpleWaveODE_CalcRHS_calc_every != TestSimpleWaveODE_CalcRHS_calc_offset)
    return;
  CCTK_INT ierr CCTK_ATTRIBUTE_UNUSED = 0;
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "TestSimpleWaveODE::evolved_grouprhs","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for TestSimpleWaveODE::evolved_grouprhs.");
  return;
}

static void TestSimpleWaveODE_CalcRHS_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(TestSimpleWaveODE_CalcRHS,
  ...
  CCTK_ENDLOOP3(TestSimpleWaveODE_CalcRHS);
}
extern "C" void TestSimpleWaveODE_CalcRHS(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering TestSimpleWaveODE_CalcRHS_Body");
  }
  if (cctk_iteration % TestSimpleWaveODE_CalcRHS_calc_every != TestSimpleWaveODE_CalcRHS_calc_offset)
  {
    return;
  ...
  AssertGroupStorage(cctkGH, "TestSimpleWaveODE_CalcRHS", 2, groups);
  
  EnsureStencilFits(cctkGH, "TestSimpleWaveODE_CalcRHS", 1, 1, 1);
  
  LoopOverInterior(cctkGH, TestSimpleWaveODE_CalcRHS_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving TestSimpleWaveODE_CalcRHS_Body");
  }
}
