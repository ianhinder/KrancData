...
/* Export definitions */
#define ML_BSSN_DGFE_convertFromADMBase_sdg_grid   hrscc::GNIGrid<hrscc::GLLElement<config_sdg_order> >
#define ML_BSSN_DGFE_convertFromADMBase_sdg_method hrscc::SDGMethod<DGFE_ML_BSSN_DGFE_convertFromADMBase, ML_BSSN_DGFE_convertFromADMBase_sdg_grid, config_riemann_solver>

/*** Numerical scheme ***/

/* Configuration */
#define config_method ML_BSSN_DGFE_convertFromADMBase_sdg_method

/* Export definitions */
#define ML_BSSN_DGFE_convertFromADMBase_method config_method
#define ML_BSSN_DGFE_convertFromADMBase_solver hrscc::CLawSolver<DGFE_ML_BSSN_DGFE_convertFromADMBase, config_method>



class DGFE_ML_BSSN_DGFE_convertFromADMBase;

} // namespace CCTK_THORN
namespace hrscc {
  using namespace CCTK_THORN;
  template<>
  struct traits<DGFE_ML_BSSN_DGFE_convertFromADMBase> {
...
class DGFE_ML_BSSN_DGFE_convertFromADMBase: public hrscc::CLaw<DGFE_ML_BSSN_DGFE_convertFromADMBase> {
public:
  typedef hrscc::CLaw<DGFE_ML_BSSN_DGFE_convertFromADMBase> claw;
  typedef hrscc::traits<DGFE_ML_BSSN_DGFE_convertFromADMBase>::state_t state_t;
  typedef hrscc::traits<DGFE_ML_BSSN_DGFE_convertFromADMBase> variables_t;
  static const int nvars = variables_t::nvars;
  
  DGFE_ML_BSSN_DGFE_convertFromADMBase();
...
namespace hrscc {
  using namespace CCTK_THORN;
  template<> int CLaw<DGFE_ML_BSSN_DGFE_convertFromADMBase>::conserved_idx[DGFE_ML_BSSN_DGFE_convertFromADMBase::nvars] = {};
  template<> int CLaw<DGFE_ML_BSSN_DGFE_convertFromADMBase>::primitive_idx[DGFE_ML_BSSN_DGFE_convertFromADMBase::nvars] = {};
  template<> int CLaw<DGFE_ML_BSSN_DGFE_convertFromADMBase>::rhs_idx[DGFE_ML_BSSN_DGFE_convertFromADMBase::nvars] = {};
  template<> int CLaw<DGFE_ML_BSSN_DGFE_convertFromADMBase>::field_idx[3*DGFE_ML_BSSN_DGFE_convertFromADMBase::nvars] = {};
  template<> int CLaw<DGFE_ML_BSSN_DGFE_convertFromADMBase>::bitmask_idx[0] = {};
} // namespace hrscc
namespace CCTK_THORN {
...
DGFE_ML_BSSN_DGFE_convertFromADMBase::DGFE_ML_BSSN_DGFE_convertFromADMBase()
{
...
static ML_BSSN_DGFE_convertFromADMBase_solver *solver = NULL;
...
static void ML_BSSN_DGFE_convertFromADMBase_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  #pragma omp parallel
  CCTK_LOOP3STR(ML_BSSN_DGFE_convertFromADMBase,
    ...
    vecimin,vecimax, CCTK_REAL_VEC_SIZE)
  {
    ...
    CCTK_REAL_VEC detg CCTK_ATTRIBUTE_UNUSED = 
      kmadd(ToReal(2),kmul(g12,kmul(g13,g23)),kmsub(g11,kmul(g22,g33),kmadd(g33,kmul(g12,g12),kmadd(g11,kmul(g23,g23),kmul(g22,kmul(g13,g13))))));
    ...
    trKL = 
      kmadd(kxxL,gu11,kmadd(ToReal(2),kmul(kxyL,gu12),kmadd(ToReal(2),kmul(kxzL,gu13),kmadd(kyyL,gu22,kmadd(ToReal(2),kmul(kyzL,gu23),kmul(kzzL,gu33))))));
  ...
  CCTK_ENDLOOP3STR(ML_BSSN_DGFE_convertFromADMBase);
}
extern "C" void ML_BSSN_DGFE_convertFromADMBase(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ML_BSSN_DGFE_convertFromADMBase_Body");
  }
  if (cctk_iteration % ML_BSSN_DGFE_convertFromADMBase_calc_every != ML_BSSN_DGFE_convertFromADMBase_calc_offset)
  {
    return;
  }
  ...
  AssertGroupStorage(cctkGH, "ML_BSSN_DGFE_convertFromADMBase", 10, groups);
  ...
  if (not solver) solver = new ML_BSSN_DGFE_convertFromADMBase_method(cctkGH);
  LoopOverEverything(cctkGH, ML_BSSN_DGFE_convertFromADMBase_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ML_BSSN_DGFE_convertFromADMBase_Body");
  }
}
