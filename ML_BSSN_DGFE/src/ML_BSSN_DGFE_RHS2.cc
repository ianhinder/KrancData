...
extern "C" void ML_BSSN_DGFE_RHS2_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % ML_BSSN_DGFE_RHS2_calc_every != ML_BSSN_DGFE_RHS2_calc_offset)
    return;
  CCTK_INT ierr CCTK_ATTRIBUTE_UNUSED = 0;
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GenericFD_GetBoundaryWidth(cctkGH), -1 /* no table */, "ML_BSSN_DGFE::ML_curvrhs","flat");
  if (ierr < 0)
...
#define config_riemann_solver hrscc::LaxFriedrichsRS<DGFE_ML_BSSN_DGFE_RHS2, false>

/* Export definitions */
#define ML_BSSN_DGFE_RHS2_sdg_grid   hrscc::GNIGrid<hrscc::GLLElement<config_sdg_order> >
#define ML_BSSN_DGFE_RHS2_sdg_method hrscc::SDGMethod<DGFE_ML_BSSN_DGFE_RHS2, ML_BSSN_DGFE_RHS2_sdg_grid, config_riemann_solver>

/*** Numerical scheme ***/

/* Configuration */
#define config_method ML_BSSN_DGFE_RHS2_sdg_method

/* Export definitions */
#define ML_BSSN_DGFE_RHS2_method config_method
#define ML_BSSN_DGFE_RHS2_solver hrscc::CLawSolver<DGFE_ML_BSSN_DGFE_RHS2, config_method>



class DGFE_ML_BSSN_DGFE_RHS2;

namespace hrscc {
  template<>
  struct traits<DGFE_ML_BSSN_DGFE_RHS2> {
    // All state vector variables
    enum state_t {iAt11, iAt12, iAt13, iAt22, iAt23, iAt33, nvars};
...
class DGFE_ML_BSSN_DGFE_RHS2: public hrscc::CLaw<DGFE_ML_BSSN_DGFE_RHS2> {
public:
  typedef hrscc::CLaw<DGFE_ML_BSSN_DGFE_RHS2> claw;
  typedef hrscc::traits<DGFE_ML_BSSN_DGFE_RHS2>::state_t state_t;
  typedef hrscc::traits<DGFE_ML_BSSN_DGFE_RHS2> variables_t;
  static const int nvars = variables_t::nvars;
  
  DGFE_ML_BSSN_DGFE_RHS2();
  ...
  inline void fluxes(hrscc::Observer<claw> & observer) const
  {
    ...
    switch (dir) {
    case hrscc::policy::x: {
      fluxAt11L = observer.field[variables_t::iAt11 + 0*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt12L = observer.field[variables_t::iAt12 + 0*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt13L = observer.field[variables_t::iAt13 + 0*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt22L = observer.field[variables_t::iAt22 + 0*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt23L = observer.field[variables_t::iAt23 + 0*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt33L = observer.field[variables_t::iAt33 + 0*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      break;
    }
    case hrscc::policy::y: {
      fluxAt11L = observer.field[variables_t::iAt11 + 1*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt12L = observer.field[variables_t::iAt12 + 1*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt13L = observer.field[variables_t::iAt13 + 1*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt22L = observer.field[variables_t::iAt22 + 1*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt23L = observer.field[variables_t::iAt23 + 1*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt33L = observer.field[variables_t::iAt33 + 1*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      break;
    }
    case hrscc::policy::z: {
      fluxAt11L = observer.field[variables_t::iAt11 + 2*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt12L = observer.field[variables_t::iAt12 + 2*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt13L = observer.field[variables_t::iAt13 + 2*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt22L = observer.field[variables_t::iAt22 + 2*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt23L = observer.field[variables_t::iAt23 + 2*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      fluxAt33L = observer.field[variables_t::iAt33 + 2*DGFE_ML_BSSN_DGFE_RHS2::nvars];
      break;
    }
    default:
...
namespace hrscc {
  template<> int CLaw<DGFE_ML_BSSN_DGFE_RHS2>::conserved_idx[DGFE_ML_BSSN_DGFE_RHS2::nvars] = {};
  template<> int CLaw<DGFE_ML_BSSN_DGFE_RHS2>::primitive_idx[DGFE_ML_BSSN_DGFE_RHS2::nvars] = {};
  template<> int CLaw<DGFE_ML_BSSN_DGFE_RHS2>::rhs_idx[DGFE_ML_BSSN_DGFE_RHS2::nvars] = {};
  template<> int CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[3*DGFE_ML_BSSN_DGFE_RHS2::nvars] = {};
  template<> int CLaw<DGFE_ML_BSSN_DGFE_RHS2>::bitmask_idx[0] = {};
} // namespace hrscc
...
DGFE_ML_BSSN_DGFE_RHS2::DGFE_ML_BSSN_DGFE_RHS2()
{
  using namespace hrscc;

  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::conserved_idx[variables_t::iAt11] = varindex(CCTK_THORNSTRING "::At11");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::conserved_idx[variables_t::iAt12] = varindex(CCTK_THORNSTRING "::At12");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::conserved_idx[variables_t::iAt13] = varindex(CCTK_THORNSTRING "::At13");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::conserved_idx[variables_t::iAt22] = varindex(CCTK_THORNSTRING "::At22");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::conserved_idx[variables_t::iAt23] = varindex(CCTK_THORNSTRING "::At23");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::conserved_idx[variables_t::iAt33] = varindex(CCTK_THORNSTRING "::At33");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::primitive_idx[variables_t::iAt11] = varindex(CCTK_THORNSTRING "::At11");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::primitive_idx[variables_t::iAt12] = varindex(CCTK_THORNSTRING "::At12");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::primitive_idx[variables_t::iAt13] = varindex(CCTK_THORNSTRING "::At13");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::primitive_idx[variables_t::iAt22] = varindex(CCTK_THORNSTRING "::At22");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::primitive_idx[variables_t::iAt23] = varindex(CCTK_THORNSTRING "::At23");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::primitive_idx[variables_t::iAt33] = varindex(CCTK_THORNSTRING "::At33");

  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt11 + 0*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt111");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt12 + 0*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt121");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt13 + 0*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt131");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt22 + 0*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt221");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt23 + 0*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt231");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt33 + 0*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt331");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt11 + 1*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt112");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt12 + 1*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt122");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt13 + 1*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt132");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt22 + 1*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt222");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt23 + 1*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt232");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt33 + 1*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt332");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt11 + 2*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt113");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt12 + 2*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt123");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt13 + 2*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt133");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt22 + 2*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt223");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt23 + 2*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt233");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::field_idx[variables_t::iAt33 + 2*DGFE_ML_BSSN_DGFE_RHS2::nvars] = varindex(CCTK_THORNSTRING "::fluxAt333");

  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::rhs_idx[variables_t::iAt11] = varindex(CCTK_THORNSTRING "::At11rhs");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::rhs_idx[variables_t::iAt12] = varindex(CCTK_THORNSTRING "::At12rhs");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::rhs_idx[variables_t::iAt13] = varindex(CCTK_THORNSTRING "::At13rhs");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::rhs_idx[variables_t::iAt22] = varindex(CCTK_THORNSTRING "::At22rhs");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::rhs_idx[variables_t::iAt23] = varindex(CCTK_THORNSTRING "::At23rhs");
  CLaw<DGFE_ML_BSSN_DGFE_RHS2>::rhs_idx[variables_t::iAt33] = varindex(CCTK_THORNSTRING "::At33rhs");
}



/* A solver, DGFE's equivalent of cctkGH */
static ML_BSSN_DGFE_RHS2_solver *solver = NULL;
...
static void ML_BSSN_DGFE_RHS2_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  CCTK_LOOP3STR(ML_BSSN_DGFE_RHS2,
    ...
    vecimin,vecimax, CCTK_REAL_VEC_SIZE)
  {
    ...
    CCTK_REAL_VEC Ats11 CCTK_ATTRIBUTE_UNUSED = 
      kmadd(Gt111,JacPDstandardNth1alpha,kmadd(Gt211,JacPDstandardNth2alpha,kmadd(Gt311,JacPDstandardNth3alpha,kmadd(alphaL,R11,kmsub(cdphi1,kmul(JacPDstandardNth1alpha,ToReal(4)),JacPDstandardNth11alpha)))));
    
    CCTK_REAL_VEC Ats12 CCTK_ATTRIBUTE_UNUSED = 
      kmadd(Gt112,JacPDstandardNth1alpha,kmadd(Gt212,JacPDstandardNth2alpha,kmadd(Gt312,JacPDstandardNth3alpha,kmadd(alphaL,R12,kmsub(kmadd(cdphi2,JacPDstandardNth1alpha,kmul(cdphi1,JacPDstandardNth2alpha)),ToReal(2),JacPDstandardNth12alpha)))));
    
    CCTK_REAL_VEC Ats13 CCTK_ATTRIBUTE_UNUSED = 
      kmadd(Gt113,JacPDstandardNth1alpha,kmadd(Gt213,JacPDstandardNth2alpha,kmadd(Gt313,JacPDstandardNth3alpha,kmadd(alphaL,R13,kmsub(kmadd(cdphi3,JacPDstandardNth1alpha,kmul(cdphi1,JacPDstandardNth3alpha)),ToReal(2),JacPDstandardNth13alpha)))));
    
    CCTK_REAL_VEC Ats22 CCTK_ATTRIBUTE_UNUSED = 
      kmadd(Gt122,JacPDstandardNth1alpha,kmadd(Gt222,JacPDstandardNth2alpha,kmadd(Gt322,JacPDstandardNth3alpha,kmadd(alphaL,R22,kmsub(cdphi2,kmul(JacPDstandardNth2alpha,ToReal(4)),JacPDstandardNth22alpha)))));
    
    CCTK_REAL_VEC Ats23 CCTK_ATTRIBUTE_UNUSED = 
      kmadd(Gt123,JacPDstandardNth1alpha,kmadd(Gt223,JacPDstandardNth2alpha,kmadd(Gt323,JacPDstandardNth3alpha,kmadd(alphaL,R23,kmsub(kmadd(cdphi3,JacPDstandardNth2alpha,kmul(cdphi2,JacPDstandardNth3alpha)),ToReal(2),JacPDstandardNth23alpha)))));
    
    CCTK_REAL_VEC Ats33 CCTK_ATTRIBUTE_UNUSED = 
      kmadd(Gt133,JacPDstandardNth1alpha,kmadd(Gt233,JacPDstandardNth2alpha,kmadd(Gt333,JacPDstandardNth3alpha,kmadd(alphaL,R33,kmsub(cdphi3,kmul(JacPDstandardNth3alpha,ToReal(4)),JacPDstandardNth33alpha)))));
    
    CCTK_REAL_VEC trAts CCTK_ATTRIBUTE_UNUSED = 
      kmadd(Ats11,gu11,kmadd(Ats22,gu22,kmadd(Ats33,gu33,kmadd(Ats12,kmul(gu12,ToReal(2)),kmadd(Ats13,kmul(gu13,ToReal(2)),kmul(Ats23,kmul(gu23,ToReal(2))))))));
    
    CCTK_REAL_VEC At11rhsL CCTK_ATTRIBUTE_UNUSED = 
      kmadd(alphaL,kmadd(At11L,trKL,kmul(kmadd(At11L,Atm11,kmadd(At12L,Atm21,kmul(At13L,Atm31))),ToReal(-2))),kmadd(At11L,kmul(kadd(JacPDstandardNth1beta1,kadd(JacPDstandardNth2beta2,JacPDstandardNth3beta3)),ToReal(-0.666666666666666666666666666667)),kmadd(em4phi,kmadd(g11,kmul(trAts,ToReal(-0.333333333333333333333333333333)),Ats11),kmadd(At11L,kmul(JacPDstandardNth1beta1,ToReal(2)),kmadd(At12L,kmul(JacPDstandardNth1beta2,ToReal(2)),kmadd(At13L,kmul(JacPDstandardNth1beta3,ToReal(2)),kmul(alphaL,kmul(em4phi,kmul(ToReal(-8),kmul(kmadd(g11,kmul(trS,ToReal(-0.333333333333333333333333333333)),eTxxL),ToReal(3.14159265358979323846264338328)))))))))));
    
    CCTK_REAL_VEC At12rhsL CCTK_ATTRIBUTE_UNUSED = 
      kmadd(At12L,JacPDstandardNth1beta1,kmadd(At22L,JacPDstandardNth1beta2,kmadd(At23L,JacPDstandardNth1beta3,kmadd(At11L,JacPDstandardNth2beta1,kmadd(At12L,JacPDstandardNth2beta2,kmadd(At13L,JacPDstandardNth2beta3,kmadd(alphaL,kmadd(At12L,trKL,kmul(kmadd(At11L,Atm12,kmadd(At12L,Atm22,kmul(At13L,Atm32))),ToReal(-2))),kmadd(At12L,kmul(kadd(JacPDstandardNth1beta1,kadd(JacPDstandardNth2beta2,JacPDstandardNth3beta3)),ToReal(-0.666666666666666666666666666667)),kmadd(em4phi,kmadd(g12,kmul(trAts,ToReal(-0.333333333333333333333333333333)),Ats12),kmul(alphaL,kmul(em4phi,kmul(ToReal(-8),kmul(kmadd(g12,kmul(trS,ToReal(-0.333333333333333333333333333333)),eTxyL),ToReal(3.14159265358979323846264338328))))))))))))));
    
    CCTK_REAL_VEC At13rhsL CCTK_ATTRIBUTE_UNUSED = 
      kmadd(At13L,JacPDstandardNth1beta1,kmadd(At23L,JacPDstandardNth1beta2,kmadd(At33L,JacPDstandardNth1beta3,kmadd(At11L,JacPDstandardNth3beta1,kmadd(At12L,JacPDstandardNth3beta2,kmadd(At13L,JacPDstandardNth3beta3,kmadd(alphaL,kmadd(At13L,trKL,kmul(kmadd(At11L,Atm13,kmadd(At12L,Atm23,kmul(At13L,Atm33))),ToReal(-2))),kmadd(At13L,kmul(kadd(JacPDstandardNth1beta1,kadd(JacPDstandardNth2beta2,JacPDstandardNth3beta3)),ToReal(-0.666666666666666666666666666667)),kmadd(em4phi,kmadd(g13,kmul(trAts,ToReal(-0.333333333333333333333333333333)),Ats13),kmul(alphaL,kmul(em4phi,kmul(ToReal(-8),kmul(kmadd(g13,kmul(trS,ToReal(-0.333333333333333333333333333333)),eTxzL),ToReal(3.14159265358979323846264338328))))))))))))));
    
    CCTK_REAL_VEC At22rhsL CCTK_ATTRIBUTE_UNUSED = 
      kmadd(alphaL,kmadd(At22L,trKL,kmul(kmadd(At12L,Atm12,kmadd(At22L,Atm22,kmul(At23L,Atm32))),ToReal(-2))),kmadd(At22L,kmul(kadd(JacPDstandardNth1beta1,kadd(JacPDstandardNth2beta2,JacPDstandardNth3beta3)),ToReal(-0.666666666666666666666666666667)),kmadd(em4phi,kmadd(g22,kmul(trAts,ToReal(-0.333333333333333333333333333333)),Ats22),kmadd(At12L,kmul(JacPDstandardNth2beta1,ToReal(2)),kmadd(At22L,kmul(JacPDstandardNth2beta2,ToReal(2)),kmadd(At23L,kmul(JacPDstandardNth2beta3,ToReal(2)),kmul(alphaL,kmul(em4phi,kmul(ToReal(-8),kmul(kmadd(g22,kmul(trS,ToReal(-0.333333333333333333333333333333)),eTyyL),ToReal(3.14159265358979323846264338328)))))))))));
    
    CCTK_REAL_VEC At23rhsL CCTK_ATTRIBUTE_UNUSED = 
      kmadd(At13L,JacPDstandardNth2beta1,kmadd(At23L,JacPDstandardNth2beta2,kmadd(At33L,JacPDstandardNth2beta3,kmadd(At12L,JacPDstandardNth3beta1,kmadd(At22L,JacPDstandardNth3beta2,kmadd(At23L,JacPDstandardNth3beta3,kmadd(alphaL,kmadd(At23L,trKL,kmul(kmadd(At12L,Atm13,kmadd(At22L,Atm23,kmul(At23L,Atm33))),ToReal(-2))),kmadd(At23L,kmul(kadd(JacPDstandardNth1beta1,kadd(JacPDstandardNth2beta2,JacPDstandardNth3beta3)),ToReal(-0.666666666666666666666666666667)),kmadd(em4phi,kmadd(g23,kmul(trAts,ToReal(-0.333333333333333333333333333333)),Ats23),kmul(alphaL,kmul(em4phi,kmul(ToReal(-8),kmul(kmadd(g23,kmul(trS,ToReal(-0.333333333333333333333333333333)),eTyzL),ToReal(3.14159265358979323846264338328))))))))))))));
  ...
  CCTK_ENDLOOP3STR(ML_BSSN_DGFE_RHS2);
}
extern "C" void ML_BSSN_DGFE_RHS2(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ML_BSSN_DGFE_RHS2_Body");
  }
  if (cctk_iteration % ML_BSSN_DGFE_RHS2_calc_every != ML_BSSN_DGFE_RHS2_calc_offset)
  {
    return;
  ...
  GenericFD_AssertGroupStorage(cctkGH, "ML_BSSN_DGFE_RHS2", 8, groups);
  
  switch (fdOrder)
  {
    case 2:
    {
      GenericFD_EnsureStencilFits(cctkGH, "ML_BSSN_DGFE_RHS2", 1, 1, 1);
      break;
    }
    
    case 4:
    {
      GenericFD_EnsureStencilFits(cctkGH, "ML_BSSN_DGFE_RHS2", 2, 2, 2);
      break;
    }
    default:
      CCTK_BUILTIN_UNREACHABLE();
  ...
  if (not solver) solver = new ML_BSSN_DGFE_RHS2_method(cctkGH);
  GenericFD_LoopOverInterior(cctkGH, ML_BSSN_DGFE_RHS2_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ML_BSSN_DGFE_RHS2_Body");
  }
}
