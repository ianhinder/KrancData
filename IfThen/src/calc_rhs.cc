...
extern "C" void calc_rhs_SelectBCs(CCTK_ARGUMENTS)
{
  ...
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GenericFD_GetBoundaryWidth(cctkGH), -1 /* no table */, "IfThen::evolved_grouprhs","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for IfThen::evolved_grouprhs.");
  return;
}
...
static void calc_rhs_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL pirhsL CCTK_ATTRIBUTE_UNUSED;
    
    if (alpha > 0)
    {
      phirhsL = piL;
      
      pirhsL = PDstandard2nd11phi + PDstandard2nd22phi + PDstandard2nd33phi;
    }
    else
    {
      phirhsL = 2*piL;
      
      pirhsL = 2*(PDstandard2nd11phi + PDstandard2nd22phi + 
        PDstandard2nd33phi);
    }
...
extern "C" void calc_rhs(CCTK_ARGUMENTS)
{
  ...
  const char* const groups[] = {
    "IfThen::evolved_group",
    "IfThen::evolved_grouprhs"};
